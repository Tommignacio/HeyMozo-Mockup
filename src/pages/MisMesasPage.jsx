...
const CLOCK_ICON = (
  <svg viewBox="0 0 24 24" className="w-6 h-6 fill-white/60 shrink-0">
...
function getMesa4Card(status) {
  if (status === 'NEW_ORDER') return { number: '4', variant: 'purple',   icon: 'cart',   status: 'Nuevo Pedido', badgeCount: 3 };
  if (status === 'COOKING')   return { number: '4', variant: 'purple',   icon: 'cart',    status: 'Esperando Comida' };
  if (status === 'OCCUPIED')  return { number: '4', variant: 'occupied', icon: 'person',  status: 'Ocupada' };
...
const CARD = 'rounded-[20px] overflow-hidden w-full max-w-sm shadow-2xl animate-[modal-in_0.25s_cubic-bezier(0.34,1.56,0.64,1)_forwards] flex flex-col h-[460px]';
...
export default function MisMesasPage({
  ...
  const mesas = [getMesa1Card(mesa1Status), getMesa2Card(mesa2Status), getMesa3Card(mesa3Released), getMesa4Card(mesa4Status), ...staticMesas];
    ...
    <div className="px-3 pt-2 pb-6 lg:px-8 lg:pb-8">
      ...
      {modal?.type === 'action' && (
        <div className={OVERLAY} onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className={CARD} style={{ background: headerColor }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="py-3 px-5 flex items-center justify-between">
              ...
              {modal.mesa.time
                ? <div className="text-[13px] text-white/80 font-medium uppercase tracking-wider">{modal.mesa.time}</div>
                : <div className="w-7" />
              }
            </div>
            ...
            <div className="flex-1 flex flex-col mx-3.5 mt-3.5 pb-4 gap-3">
              ...
              {modal.mesa.alert && (
                <div className="bg-white/10 rounded-[14px] flex flex-col items-center py-7 gap-3 shrink-0">
                  <div className="text-[60px] leading-none">{modal.mesa.alert.emoji}</div>
                  <div className="text-[22px] font-bold text-white tracking-wide text-center px-4">
              ...
              <div className="flex-1 bg-white/10 rounded-[14px] overflow-y-auto">
                {(mesaHistory[modal.mesa.number] ?? []).map((ev, i, arr) => (
                  <div
                    key={i}
                    className={`flex items-center gap-4 py-[16px] px-5 ${i < arr.length - 1 ? 'border-b border-white/10' : ''}`}
                  >
                    {ev.icon === 'bell' ? BELL_ICON_SM : CLOCK_ICON}
                    <div className="flex-1">
                      <div className="text-[15px] font-bold text-white leading-tight">{ev.event}</div>
                      <div className="text-[12px] text-white/60 mt-0.5">{ev.time}</div>
                    </div>
                    {ev.emoji && (
        ...
        <div className={OVERLAY} onClick={(e) => e.target === e.currentTarget && setModal({ type: 'action', mesa: modal.mesa })}>
          <div className={CARD} style={{ background: headerColor }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="py-3 px-5 text-center">
              <div className="text-[20px] font-bold text-white tracking-wide uppercase">Confirmar</div>
            </div>

            {/* Confirmation body */}
            <div className="flex-1 bg-white/10 mx-3.5 mt-3.5 rounded-[14px] flex flex-col items-center justify-center px-5 gap-3 text-center">
              <div className="text-[26px] font-bold text-white leading-tight">
                ¿LIBERAR<br />MESA {modal.mesa.number}?
              </div>
              <div className="text-[14px] text-white/60 leading-snug">
            ...
            <button
              type="button"
              className="flex items-center justify-center gap-1.5 pt-3 px-5 pb-1 text-white/60 text-sm font-medium cursor-pointer bg-transparent border-none w-full font-[inherit]"
              onClick={() => setModal({ type: 'action', mesa: modal.mesa })}
            >
              CANCELAR
            </button>

            {/* CONFIRMAR — white button */}
            <div className="mt-auto flex justify-center pb-4">
              <button
                type="button"
                className="w-[80%] py-4 rounded-[12px] text-center text-[18px] font-bold bg-white text-[#1a1a1a] cursor-pointer tracking-wide transition-opacity active:opacity-75 border-none font-[inherit]"
